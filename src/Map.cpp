...
//            bool bIsThisKFRedundant = (count >= 2);
            ...
            if(bIsThisKFRedundant) {
                ...
                if(lastKF && nextKF && !bIsInitKF && !bHasFeatEdge) {
                    ...
                    Se2 dOdoLastThis = thisKF->mOdo - lastKF->mOdo;
                    Se2 dOdoThisNext = nextKF->mOdo - thisKF->mOdo;
                    ...
                    if (dl1 < theshl &&  dl2 < theshl && dt1 < thesht && dt2 < thesht) {
                        ...
                        Mat measure;
                        g2o::Matrix6d info;
                        Track::calcOdoConstraintCam(nextKF->mOdo - lastKF->mOdo, measure, info);
...
void Map::optimizeLocalGraph(SlamOptimizer &optimizer){
    ...
    for(int i = 0; i < nLocalKFs; i++) {
        ...
        Mat Tcw = toCvMat(estimateVertexSE3Expmap(optimizer, i));
        pKF->setPose(Tcw);
    }
