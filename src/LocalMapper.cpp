...
void LocalMapper::addNewKF(PtrKeyFrame& pKF, const vector<Point3f> &localMPs,
...

    {
        ...
        // Add KeyFrame-KeyFrame relation
        {
            ...
            Mat measure;
            g2o::Matrix6d info;
            Track::calcOdoConstraintCam(pKF->mOdo - pKF0->mOdo, measure, info);
...
void LocalMapper::findCorrespd(const vector<int> &vMatched12, const vector<Point3f> &localMPs, const vector<bool>& vbGoodPrl){
    ...
    if(!bNoMP) {
        ...
        for(int i = 0, iend = pPrefKF->N; i < iend; i++) {
            if(pPrefKF->hasObservation(i) && vMatched12[i]>=0){
                ...
                mNewKF->setViewMP( scv::se3map(mNewKF->Tcr, pPrefKF->mViewMPs[i]), vMatched12[i], xyzinfo );
    ...
    if(!bNoMP) {
        ...
        for(int i = 0; i < mNewKF->N; i++){
            ...
            Point3f x3d =
            scv::triangulate(pMP->getMainMeasure(), mNewKF->keyPointsUn[i].pt,
                             Config::Kcam*pMP->mMainKF->Tcw.rowRange(0,3),
                             Config::Kcam*mNewKF->Tcw.rowRange(0,3));
            Point3f posNewKF = scv::se3map(mNewKF->Tcw, x3d);
    ...
    for(int i = 0, iend = pPrefKF->N; i < iend; i++){
        if(!pPrefKF->hasObservation(i) && vMatched12[i]>=0){
            ...
            Point3f posW = scv::se3map(scv::inv(pPrefKF->Tcw), localMPs[i]);
            Point3f posKF = scv::se3map(mNewKF->Tcr, localMPs[i]);
            Eigen::Matrix3d xyzinfo, xyzinfo0;
...
#ifdef TIME_TO_LOG_LOCAL_BA
    local_ba_time_log.open("/home/fzheng/Documents/se2clam_lobal_time.txt");
#endif
