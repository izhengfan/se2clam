...
void MapPoint::updateParallax(const PtrKeyFrame &pKF){
    ...
    Point3f posW = scv::triangulate(pt0, pt1, P0, P1);

    Point3f pos0 = scv::se3map(pKF0->Tcw, posW);
    Point3f pos1 = scv::se3map(pKF->Tcw, posW);
    if(Config::acceptDepth(pos0.z) && Config::acceptDepth(pos1.z)) {
        Point3f Ocam0 = Point3f( scv::inv(pKF0->Tcw).rowRange(0,3).col(3) );
        Point3f Ocam1 = Point3f( scv::inv(pKF->Tcw).rowRange(0,3).col(3) );
        if( scv::checkParallax(Ocam0, Ocam1, posW, 2) ) {
            mPos = posW;
            mbGoodParallax = true;
            ...
            for(auto it = mObservations.begin(), iend = mObservations.end(); it!=iend; it++){
                ...
                Point3f posk = scv::se3map(pKF_k->Tcw, posW);
...
void MapPoint::updateMeasureInKFs(){
    ...
    for(auto it = mObservations.begin(), iend = mObservations.end(); it!=iend; it++) {
        ...
        Mat Tcw = pKF->getPose();
        Point3f posKF = scv::se3map(Tcw, mPos);
