...
MapPublish::MapPublish(Map* pMap){
    ...
    const char* MAP_FRAME_ID = "/se2clam/World";
    ...
    tf::Transform tfT;
    tfT.setIdentity();
    tfb.sendTransform(tf::StampedTransform(tfT,ros::Time::now(),"/se2clam/World","/se2clam/Camera"));

    publisher = nh.advertise<visualization_msgs::Marker>("se2clam/Map", 10);
...
void MapPublish::PublishCameraCurr(const cv::Mat &Twc)
{
    ...
    tfb.sendTransform(tf::StampedTransform(tfwTc,ros::Time::now(),"se2clam/World","se2clam/Camera"));
